...
def parse_rule_old(rule: dict) -> Rule:
    _rule = None
    proto = rule["protocol"]
    try:
        _rule = Rule(chains[proto], rule["id"])
    ...
    if type(src) is list:
        ...
        for item in src:
            try:
                sets["{}_v4".format(item)]
                srcList.append("@{}_v4".format(item))
                found = True
            except KeyError:
                pass
            
            try:
                sets["{}_v6".format(item)]
                srcList.append("@{}_v6".format(item))
                found = True
            except KeyError:
                pass
        if not found:
        ...
        if not found:
            raise Exception("Invalid source: {}".format(src))
    if found:
        _match = Match({"payload": {"protocol": "ip", "field": "saddr"}}, "in", srcList)
        _rule.add_match(_match)
    ...
    if type(dst) is list:
        ...
        for item in dst:
            try:
                sets["{}_v4".format(item)]
                dstList.append("@{}_v4".format(item))
                found = True
            except KeyError:
                pass
            ...
            try:
                sets["{}_v6".format(item)]
                dstList.append("@{}_v6".format(item))
                found = True
            except KeyError:
                pass
        if not found:
            raise Exception("Invalid destination: {}".format(dst))
    elif dst:
        dstList = []
        try:
            sets["{}_v4".format(dst)]
            dstList.append("@{}_v4".format(dst))
            found = True
        except KeyError:
            pass
        ...
        if not found:
            raise Exception("Invalid destination: {}".format(dst))
    if found:
        _match = Match({"payload": {"protocol": "ip", "field": "daddr"}}, "in", dstList)
        _rule.add_match(_match)
    ...
    if sport is list:
        ...
        for item in sport:
            try:
                sets[item]
                if sets[item].protocol == proto:
                    sportList.append("@{}".format(item))
                    found = True
            except KeyError:
                try:
                    sets["{}_{}".format(item, proto)]
                    sportList.append("@{}_{}".format(item, proto))
                    found = True
                except KeyError:
                    pass
        if not found:
            raise Exception("Invalid sport: {}".format(sport))
    elif sport:
        sportList = []
        try:
            sets[sport]
            ...
            try:
                sets["{}_{}".format(sport, proto)]
                sportList.append("@{}_{}".format(sport, proto))
                found = True
            except KeyError:
                pass
        if not found:
            raise Exception("Invalid sport: {}".format(sport))
    if found:
        _match = Match({"payload": {"protocol": proto, "field": "sport"}}, "in", sportList)
        _rule.add_match(_match)
    ...
    if dport is list:
        ...
        for item in dport:
            try:
                sets[item]
                if sets[item].protocol == proto:
                    dportList.append("@{}".format(item))
                    found = True
            except KeyError:
                try:
                    sets["{}_{}".format(item, proto)]
                    dportList.append("@{}_{}".format(item, proto))
                    found = True
                except KeyError:
                    pass
        if not found:
        ...
        try:
            ...
            if sets[dport].protocol == proto:
                dportList.append("@{}".format(dport))
                found = True
        except KeyError:
            try:
                sets["{}_{}".format(dport, proto)]
                dportList.append("@{}_{}".format(dport, proto))
                found = True
            except KeyError:
                pass
        if not found:
            raise Exception("Invalid dport: {}".format(dport))
    if found:
        _match = Match({"payload": {"protocol": proto, "field": "dport"}}, "in", dportList)
        _rule.add_match(_match)
